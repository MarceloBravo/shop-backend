import { AtributosProductoModel } from "../../models/AtributosProductoModel.js";

export const getAtributoProducto = async (id) => {
    const data = await AtributosProductoModel.findByPk(id, {
        include: [
            {
                model: AtributosModel, // Modelo relacionado
                as: 'atributos'
            }
        ]
    });

    return (data && data.deleted_at == null) ? data : null;
}


export const getAllAtributoProducto = async () => {
    const { rows, count } = await AtributosProductoModel.findAndCountAll({
        where: {deleted_at: null},
        order: [['producto_id','ASC']],
        include: [
            {
                model: AtributosModel, // Modelo relacionado
                as: 'atributos'
            }
        ]
    });
    return {data: rows, count};
}


export const getPageAtributoProducto = async (desde = 1, regPorPag = 10) => {
    const { rows , count } = await AtributosProductoModel.findAndCountAll({
        where: {deleted_at: null},
        offset:desde,
        limit: regPorPag,
        order: [['producto_id','ASC']]
    });    
    return {rows, count, totPag: Math.ceil(count / regPorPag)};
}


export const createAtributoProducto = async (data, transaction) => {
    const newRecord = await AtributosProductoModel.create(data, {transaction});
    return newRecord;
}


export const updateAtributoProducto = async (id, data, transaction) => {
    const [record, created] = await AtributosProductoModel.findOrCreate({
        where: {id},
        defaults: data, 
        transaction
    });
    record.producto_id = data.producto_id;
    record.atributo_id = data.atributo_id;
    if (created) return { data: record, created };

    // Si el registro ya existe, actualiza los valores
    record.deleted_at = null;

    await record.save();

    return {data: record, created};
}


export const deleteAtributoProducto = async (id, transaction) => {
    const result = await AtributosProductoModel.destroy({where: {id}},{transaction});
    return {id, result};
}


export const softDeleteAtributoProducto = async (id) => {
    const record = await AtributosProductoModel.findByPk(id);
    const eliminado = (record && record.deleted_at !== null);
    
    if(eliminado === null || eliminado === true)return null;

    record.deleted_at = new Date();
    await record.save();
    return record;
}


export const isCheckInUse = async (idAtributo) => {
    const relacionesRestantes = await AtributosProductoModel.count({ where: { atributo_id: idAtributo } });
    return relacionesRestantes > 0;
}