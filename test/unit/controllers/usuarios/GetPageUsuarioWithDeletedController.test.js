...
import GetPageUsuarioWithDeletedController from '../../../../src/controllers/usuario/GetPageUsuarioWithDeletedController.js';
...
describe('GetPageUsuarioWithDeletedController', () => {
  let getPageUsuarioWithDeletedController;
  let mockRequest;
  let mockResponse;
  ...
  it('should return a page of usuarios including deleted ones', async () => {
    const mockRows = [{ id: 1 }, { id: 2, deleted_at: new Date() }];
    const mockCount = 20;
    const mockTotPag = 2;
    ...
    await getPageUsuarioWithDeletedController.execute(mockRequest, mockResponse);

    expect(mockGetPageService.execute).toHaveBeenCalledWith(1, 10, false);
  ...
  it('throw a error if none repository is provided', () => {
    expect(() => new GetPageUsuarioWithDeletedController()).toThrow('No se ha recibido un repositorio');
  });
});
