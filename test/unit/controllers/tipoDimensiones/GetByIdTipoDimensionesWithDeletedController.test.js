import GetByIdTipoDimensionesWithDeletedController from '../../../../src/controllers/tipoDimensiones/GetByIdTipoDimensionesWithDeletedController.js';
...
describe('GetByIdTipoDimensionesWithDeletedController', () => {
  let getByIdTipoDimensionesWithDeletedController;
  let mockRequest;
  let mockResponse;
  ...
  it('should return a tipo dimensiones by ID including deleted ones', async () => {
    const tipoDimensiones = { id: 1, nombre: 'Test', deleted_at: new Date() };

    mockRequest.params.id = 1;
    mockGetByIdService.execute.mockResolvedValue(tipoDimensiones);

    await getByIdTipoDimensionesWithDeletedController.execute(mockRequest, mockResponse);
