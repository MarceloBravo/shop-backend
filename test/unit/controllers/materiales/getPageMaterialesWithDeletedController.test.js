...
import GetPageMaterialesWithDeletedController from '../../../../src/controllers/materiales/GetPageMaterialWithDeletedController.js';

describe('Unit Test: GetPageMaterialesWithDeletedController', () => {
    let controller;

    beforeEach(() => {
        jest.clearAllMocks();
        controller = new GetPageMaterialesWithDeletedController(mockRepository);
    });

    it('Obtiene una página de materialeses (incluyendo eliminados) exitosamente', async () => {
        const pag = 1, limit = 2;
        const rows = [
            { id: 1, valor: 'Algodón' },
            { id: 2, valor: 'Cuero' }
