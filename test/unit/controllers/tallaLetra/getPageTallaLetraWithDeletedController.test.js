...
import GetPageTallaLetraWithDeletedController from '../../../../src/controllers/tallaLetra/GetPageTallaLetraWithDeletedController.js';

describe('Unit Test: GetPageTallaLetraWithDeletedController', () => {
    let controller;

    beforeEach(() => {
        jest.clearAllMocks();
        controller = new GetPageTallaLetraWithDeletedController(mockRepository);
    });

    it('Obtiene una página de tallaLetraes (incluyendo eliminados) exitosamente', async () => {
        // Arrange
        const mockRows = [{ id: 1, valor: 'M' }];
        const mockCount = 1;
        const mockTotPag = 1;
