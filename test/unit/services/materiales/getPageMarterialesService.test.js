...
import GetPageMaterialesService from '../../../../src/services/materiales/GetPageMaterialService.js';

describe('Unit Test: GetPageMaterialesService', () => {
    let service;

    beforeEach(() => {
        // Limpia todos los mocks antes de cada test
        jest.clearAllMocks();
        service = new GetPageMaterialesService(mockRepository);
    });

    it('Obtiene una página de materiales exitosamente', async () => {
        // Arrange
        const mockMateriales = [
            { id: 1, valor: 'Cuero' },
            { id: 2, valor: 'Lana' }
        ];
        const mockResponse = { rows: mockMateriales, count: 10, totPag: 2 };
    ...
    it('Obtiene una página de materialess incluyendo eliminadas', async () => {
        // Arrange
        const mockMaterialess = [
            { id: 1, valor: 'Cuero' },
            { id: 2, valor: 'Lana', deletedAt: '2023-01-01' }
        ];
        const mockResponse = { rows: mockMaterialess, count: 15, totPag: 3 };
        mockRepository.getPage.mockResolvedValue(mockResponse);
    ...
    it('Calcula correctamente totPag para diferentes cantidades', async () => {
        // Arrange
        const mockResponse = { rows: [], count: 25, totPag: 3};
        mockRepository.getPage.mockResolvedValue(mockResponse);
    ...
    it('Calcula totPag como 1 cuando count es menor que limit', async () => {
        // Arrange
        const mockResponse = { rows: [], count: 5, totPag: 1 };
        mockRepository.getPage.mockResolvedValue(mockResponse);
    ...
    it('Lanza error si no se proporciona un repositorio', () => {
        expect(() => new GetPageMaterialesService()).toThrow('El repositorio es requerido');
    });
}); 